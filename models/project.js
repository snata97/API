    ...
    return db.task(t => {
        return t.one('SELECT * FROM project WHERE id=${id}', {id: id})
            .then(function(){
                    ...
                    `SELECT acc.id, acc.firstname, acc.lastname, acc.farthername, acc.photo,
                        array_to_json(array_agg(json_build_object('name',job.name))) as jobs
                        FROM account_in_project AS acc_proj
                        LEFT JOIN account AS acc ON acc_proj.accountid = acc.id
                        LEFT JOIN job AS job ON acc_proj.jobid = job.id
                        WHERE acc_proj.projectid=${id}
                        GROUP BY acc.id;`+
                    `SELECT vacancy.id, job.name, vacancy.description, vacancy.count
                        FROM vacancy
                        LEFT JOIN job AS job ON vacancy.jobid = job.id
                        WHERE vacancy.projectid=${id};`+
                    `SELECT event.id, event.name, event.photo, event.date
                        FROM event
                        LEFT JOIN project ON project.id = event.projectid
